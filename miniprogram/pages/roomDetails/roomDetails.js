...
create(store, {
  ...
  _onExtraArr() {
    if(this.data._countS == 1){
      this.store.data.extraArr.push({ item: '朝阳房', price: 30 });
    }
    if(this.data._countP == 1) {
      this.store.data.extraArr.push({ item: '布宫窗景房', price: 30 });
    }
    if(this.data._countO == 1) {
      this.store.data.extraArr.push({ item: '吸氧设施', price: 50 });
    }
    if(this.data._countB == 1){
      this.store.data.extraArr.push({ item: '加床', price: 100 });
    }
    this.update();
  },
  ...
  onAddCart() {
    ...
    db.collection('cart').add({
      data: {
        ...
        roomImg: this.store.data.roomType.headImg,
